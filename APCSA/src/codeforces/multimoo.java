	...
	static class traversalHelper {
		...
		public int findMaxOcc(int r, int c, int target) {
			...
			if (!visited[r][c] && board[r][c] == target) {
				tempmax++;
				visited[r][c] = true;
				if (board[r + 1][c] == target && !visited[r + 1][c])
					tempmax += findMaxOcc(r + 1, c, target);
				if (board[r - 1][c] == target && !visited[r - 1][c])
					tempmax += findMaxOcc(r - 1, c, target);
				if (board[r][c + 1] == target && !visited[r][c + 1])
					tempmax += findMaxOcc(r, c + 1, target);
				if (board[r][c - 1] == target && !visited[r][c - 1])
					tempmax += findMaxOcc(r, c - 1, target);
			}
